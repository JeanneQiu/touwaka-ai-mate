# 项目待办事项

> 最后更新：2026-02-23

---

## 待开始

| 任务 | 状态 | 优先级 |
|------|------|--------|
| [工具调用可视化面板 + SearXNG 搜索技能](tasks/tool-visualization.md) | ⏳ 待开始 | 中 |
| [专家 LLM 参数配置化](tasks/expert-llm-params.md) | ⏳ 待开始 | 中 |
| [反思心智模板配置化](tasks/reflective-mind-template.md) | ⏳ 待开始 | 中 |

---

## 进行中

| 任务 | 状态 | 优先级 |
|------|------|--------|
| [Skill 管理系统](tasks/skill-management.md) | 🔄 进行中 | 高 |
| [对话窗口右侧多功能 Panel](tasks/right-panel.md) | ✅ 基础功能已完成 | 中 |

---

## 待开始（高优先级）

| 任务 | 状态 | 优先级 |
|------|------|--------|
| [上下文压缩与话题总结重构](tasks/context-compression.md) | 🔄 基础设施已完成 | 高 |

---

## 文档索引

### 任务详情
- [tasks/tool-visualization.md](tasks/tool-visualization.md) - 工具调用可视化面板
- [tasks/expert-llm-params.md](tasks/expert-llm-params.md) - 专家 LLM 参数配置化
- [tasks/reflective-mind-template.md](tasks/reflective-mind-template.md) - 反思心智模板配置化
- [tasks/skill-management.md](tasks/skill-management.md) - Skill 管理系统
- [tasks/right-panel.md](tasks/right-panel.md) - 右侧面板
- [tasks/context-compression.md](tasks/context-compression.md) - 上下文压缩重构

### 数据库手册
| 文档 | 描述 |
|------|------|
| [README.md](../guides/database/README.md) | 数据库概览与快速开始 |
| [api-query-design.md](../guides/database/api-query-design.md) | 复杂查询 API 规范 |
| [orm-analysis.md](../guides/database/orm-analysis.md) | ORM 选型分析 |

### 设计文档
| 文档 | 描述 |
|------|------|
| [context-compression-design.md](../design/v2/context-compression-design.md) | ⭐ 上下文压缩设计 v2 |
| [llm-call-scenarios.md](../design/v1/llm-call-scenarios.md) | LLM 调用场景分析 |
| [background-task-scheduler-design.md](../design/v2/background-task-scheduler-design.md) | 后台任务调度器设计 |
| [right-panel-design.md](../design/v2/right-panel-design.md) | 右侧面板容器设计 |
| [task-layer-design.md](../design/v2/task-layer-design.md) | 任务层设计 |
| [skill-market-design.md](../design/v2/skill-market-design.md) | 技能管理系统设计 |

---

*使用说明：状态图标含义*
- 🔄 进行中
- ⏳ 待开始  
- ✅ 已完成
- ❌ 已取消
