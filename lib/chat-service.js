...
class ChatService {
  ...
  async streamChat(params, onDelta, onComplete, onError) {
    ...
    try {
      ...
      // 2. 获取或创建活跃对话（Topic 完全由后端管理）
      const topic_id = providedTopicId || await this.getOrCreateActiveTopic(user_id, expert_id);
      logger.debug('[ChatService] Topic ID:', topic_id);

      // 3. 发送开始事件
      onDelta?.({ type: 'start', message_id: `msg_${Utils.newID(10)}`, topic_id });
      ...
      await expertService.llmClient.callStream(
        ...
        context.messages,
        {
          tools: expertService.toolManager.getToolDefinitions(),
          onDelta: (delta) => {
            fullContent += delta;
            tokenCount++;
      ...
      onComplete?.({
        ...
        content: fullContent,
        usage: { prompt: context.messages.length * 10, completion: tokenCount },
        latency,
  ...
  async chat(params) {
    ...
    try {
      ...
      // 7. 异步执行反思
      expertService.performReflection(user_id, content, response).catch(err => {
        logger.error('[ChatService] 反思失败:', err.message);
      });
  ...
  async saveUserMessage(topic_id, user_id, content, expert_id = null) {
    ...
    // 更新 Topic 消息计数
    await this.incrementTopicMessageCount(topic_id);
  ...
  async processUnrepliedMessages() {
    try {
      ...
      // 使用原生 SQL 查询（复杂子查询用 Sequelize 实现较困难）
      ...
      for (const msg of unrepliedMessages) {
        try {
          logger.info(`[ChatService] 处理未回复消息: ${msg.id}, topic: ${msg.topic_id}`);
...
class ExpertChatService {
   ...
   * 执行反思（异步）
   */
  async performReflection(user_id, triggerMessage, myResponse) {
    try {
      // 获取最近消息作为上下文
   ...
   * 当消息积累到一定数量后，由 MemorySystem 自动总结并更新 Topic 标题
   * @param {string} user_id - 用户ID
   * @param {string} topic_id - 当前话题ID（可选，用于更新标题）
   */
  async processHistoryIfNeeded(user_id, topic_id = null) {
    try {
      // 检查是否需要归档
      const shouldArchive = await this.memorySystem.shouldProcessHistory(user_id);
      
      if (shouldArchive) {
        logger.info(`[ExpertChatService] [${this.expertName}] 开始历史归档: user=${user_id}, topic=${topic_id}`);
        await this.memorySystem.processHistory(user_id, topic_id);
        logger.info(`[ExpertChatService] [${this.expertName}] 历史归档完成: user=${user_id}`);
      }
    } catch (error) {
      logger.error(`[ExpertChatService] [${this.expertName}] 历史归档失败:`, error.message);
      throw error;
    }
  }
