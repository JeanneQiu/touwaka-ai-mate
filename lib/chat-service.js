...
class ChatService {
  ...
  async streamChat(params, onDelta, onComplete, onError) {
    const { topic_id: providedTopicId, user_id, expert_id, content, model_id, task_id } = params;

    try {
      logger.info('[ChatService] 开始流式聊天:', { expert_id, user_id, topic_id: providedTopicId, task_id });
      ...
      if (task_id) {
        taskContext = await this.getTaskContext(task_id, user_id);
        if (taskContext) {
          logger.info('[ChatService] 任务上下文已加载:', taskContext.title);
        }
      }
   ...
   */
  async getTaskContext(task_id, user_id) {
    try {
      if (!this.Task) {
      ...
      // 获取工作空间文件列表（仅 input 目录）
      const fs = await import('fs/promises');
      const path = await import('path');
      
      try {
        const inputDir = path.join(task.workspace_path, 'input');
