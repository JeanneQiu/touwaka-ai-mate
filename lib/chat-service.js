...
class ExpertChatService {
  ...
  async performReflection(user_id, triggerMessage, myResponse, topic_id = null) {
    try {
      ...
      if (topic_id) {
        ...
        if (topic) {
          // 获取当前话题已累积的关键词
          const currentKeywords = this.getCurrentTopicKeywords(user_id, topic_id);
          topicInfo = {
            title: topic.title,
      ...
      if (reflection.keywords && reflection.keywords.length > 0 && topic_id) {
        this.accumulateKeywords(user_id, topic_id, reflection.keywords);
      }
  ...
  accumulateKeywords(user_id, topic_id, newKeywords) {
    ...
    logger.debug(`[ExpertChatService] 话题关键词累积: ${topic_id}, 当前 ${merged.length} 个: ${merged.join(', ')}`);
  }
