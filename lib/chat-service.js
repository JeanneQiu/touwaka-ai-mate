...
class ChatService {
  ...
  async saveToolMessage(topic_id, user_id, toolResult, expert_id = null) {
    ...
    // 格式: { tool_call_id, name, content, success }
    const toolCallsData = {
    ...
    await this.Message.create({
      ...
      expert_id,
      role: 'tool',
      content: toolResult.result || toolResult.error || '',  // 工具返回内容存入 content
      tool_calls: JSON.stringify(toolCallsData),  // 工具元数据复用 tool_calls 字段
    });
