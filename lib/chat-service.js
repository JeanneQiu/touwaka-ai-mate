...
class ChatService {
  ...
  async streamChat(params, onDelta, onComplete, onError) {
    ...
    try {
      ...
      // 如果用户提供了 topic_id，直接使用；否则检测是否需要切换话题
      let topic_id = providedTopicId;
      let isNewTopic = false;

      if (!topic_id) {
        // 检测是否需要切换话题
        const topicCheck = await this.checkAndHandleTopicShift(
          user_id,
          expert_id,
          content,
          expertService
        );
        topic_id = topicCheck.topic_id;
        isNewTopic = topicCheck.isNewTopic;
      }

      logger.debug('[ChatService] Topic ID:', topic_id, isNewTopic ? '(新话题)' : '(继续当前话题)');

      // 3. 发送开始事件（包含是否为新话题的信息）
