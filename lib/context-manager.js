...
class ContextManager {
   ...
   * @param {boolean} options.includeTopics - 是否包含 Topic 上下文（默认 true）
  ...
  async buildContext(memorySystem, userId, options = {}) {
    ...
    // 2. 获取最近消息（用于对话历史）
    const recentMessages = await memorySystem.getRecentMessages(
      userId,
    ...
    if (includeTopics) {
      topicContext = await this.buildTopicContext(memorySystem, userId, currentMessage);
    }
    ...
    const conversationCount = Math.floor(recentMessages.length / 2); // 估算对话轮次
    const userInfoGuidance = this.generateUserInfoGuidance(userProfileContext, conversationCount);

    // 7. 构建系统提示（包含引导提示）
    const systemPrompt = this.buildSystemPrompt(innerVoices, userInfoGuidance);
