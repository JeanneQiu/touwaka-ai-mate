...
import { formatListField } from './utils/prompt-utils.js';
...
`;
   ...
   *
   * 空字段处理逻辑：
   * - 空数组 `[]`、`null`、`undefined`、空字符串均视为空
   * - 空字段对应的章节标题也会被省略，不会输出 "未定义" 或空白内容
  ...
  enhanceWithSoul(systemPrompt, soul, expert = null) {
    ...
    // 核心价值观
    const coreValues = formatListField(soul.coreValues);
    if (coreValues) {
      sections.push(`## 你的核心价值观\n${coreValues}`);
    }

    // 行为准则
    const guidelines = formatListField(soul.behavioralGuidelines);
    if (guidelines) {
      sections.push(`## 你的行为准则\n${guidelines}`);
    }

    // 禁忌
    const taboos = formatListField(soul.taboos);
    if (taboos) {
      sections.push(`## 你的禁忌（绝对不能做的事）\n${taboos}`);
    }

    // 情感基调
    if (soul.emotionalTone) {
