...
class ContextManager {
   ...
   * 2. Topic Summaries（话题总结，从 topics 表加载）
   * 3. Unarchived Messages（未归档消息，topic_id IS NULL）
   *
   ...
   * @param {string} userInfoGuidance - 用户信息引导提示（可选）
   */
  buildSystemPromptWithTopics(innerVoices = [], topicSummaries = null, userInfoGuidance = null) {
    const expert = this.expertConfig.expert || this.expertConfig;
    ...
    // 添加 Topic 总结（新设计：在 Soul 之后、Inner Voices 之前）
    if (topicSummaries) {
