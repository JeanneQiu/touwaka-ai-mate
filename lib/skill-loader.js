...
class SkillLoader {
  ...
  async loadSkillFromDatabase(skillRow) {
    ...
    const tools = await this.loadSkillTools(skillRow.id);
  ...
  async loadSkillTools(skillId) {
    try {
      ...
      logger.info(`[SkillLoader] 从 skill_tools 表加载了 ${toolRows.length} 个工具 for skill ${skillId}`);

      return toolRows.map(row => this.convertToolToOpenAIFormat(row, skillId));
    } catch (error) {
      logger.error(`[SkillLoader] 加载技能 ${skillId} 的工具失败:`, error.message);
      return [];
    }
  }
   ...
   */
  convertToolToOpenAIFormat(toolRow, skillId) {
    // 解析 usage 字段作为参数定义
    ...
    return {
      type: 'function',
      function: {
        name: `${skillId}_${toolRow.name}`,  // 命名空间避免冲突
