...
class SkillLoader {
  ...
  buildSkillEnvironment(skillId, config, sourcePath = null) {
    ...
    // 2. 如果 source_path 以 "data/skills/" 开头，基于项目根目录
    // 3. 如果 source_path 以 "skills/" 开头，补上 "data/" 前缀
    // 4. 否则 source_path 只是技能目录名，拼接 skillsBasePath
    let skillPath;
    if (sourcePath) {
      if (path.isAbsolute(sourcePath)) {
        // 绝对路径，直接使用
        skillPath = sourcePath;
      } else if (sourcePath.startsWith('data/skills/')) {
        // 完整相对路径 "data/skills/xxx"，基于项目根目录
        skillPath = path.join(process.cwd(), sourcePath);
      } else if (sourcePath.startsWith('skills/')) {
        // 缺少 "data/" 前缀，补上
        skillPath = path.join(process.cwd(), 'data', sourcePath);
      } else {
