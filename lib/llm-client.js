...
class LLMClient {
  ...
  async call(model, messages, options = {}) {
    ...
    // 优先使用 options 中的 timeout，否则使用模型配置的 timeout
    const timeoutValue = options.timeout || model.timeout || 60000;
  ...
  async callStream(model, messages, options = {}) {
    const { tools, onDelta, onToolCall } = options;
    ...
    return new Promise((resolve, reject) => {
      ...
      req.on('error', (error) => {
        logger.error('[LLMClient] 流式请求错误:', error.message);
        reject(error);
      });
