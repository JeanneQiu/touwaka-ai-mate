...
class LLMClient {
  ...
  async callExpressive(messages, options = {}) {
    ...
    return this.callWithRetry(model, messages, {
      ...
      top_p: expert?.top_p ?? 1.0,
      frequency_penalty: expert?.frequency_penalty ?? 0.0,
      presence_penalty: expert?.presence_penalty ?? 0.0,
      ...options,
    });
  }
  ...
  async callReflective(messages, options = {}) {
    ...
    logger.info('[LLMClient] 准备调用 Reflective Mind:', {
      ...
      temperature: expert?.reflective_temperature ?? 0.3,
    });
    return this.callWithRetry(model, messages, {
      ...options,
      temperature: expert?.reflective_temperature ?? 0.3, // 使用专家配置的反思温度
      top_p: expert?.top_p ?? 1.0,
      frequency_penalty: expert?.frequency_penalty ?? 0.0,
      presence_penalty: expert?.presence_penalty ?? 0.0,
  ...
  async callStream(model, messages, options = {}) {
    ...
    // 分析消息构成
    const messageAnalysis = this.analyzeMessages(messages);
    ...
    // 在控制台输出上下文使用情况
    console.log('\n========================================');
    console.log(`[上下文评估] 模型: ${model.model_name}`);
    console.log(`[上下文评估] 当前对话 Token: ${estimatedTokens}`);
    console.log(`[上下文评估] 模型最大上下文: ${maxContext}`);
    console.log(`[上下文评估] 使用比例: ${ratio}%`);
    console.log('----------------------------------------');
    console.log(`[消息构成] 总消息数: ${messages.length}`);
    console.log(`[消息构成] 系统消息: ${messageAnalysis.system.count} 条, ${messageAnalysis.system.chars} 字符, ~${messageAnalysis.system.tokens} tokens`);
    console.log(`[消息构成] 历史对话: ${messageAnalysis.history.count} 条, ${messageAnalysis.history.chars} 字符, ~${messageAnalysis.history.tokens} tokens`);
    if (messageAnalysis.toolResults.count > 0) {
      console.log(`[消息构成] 工具结果: ${messageAnalysis.toolResults.count} 条, ${messageAnalysis.toolResults.chars} 字符, ~${messageAnalysis.toolResults.tokens} tokens`);
    }
    console.log('========================================\n');
