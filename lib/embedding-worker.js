    ...
    try {
      ...
      if (successCount > 0) {
        ...
        for (const knowledgeId of affectedKnowledgeIds) {
          try {
            // 统计该知识的知识点向量化情况
            const totalPoints = await models.KnowledgePoint.count({
              where: { knowledge_id: knowledgeId },
            });
            const vectorizedPoints = await models.KnowledgePoint.count({
              where: { knowledge_id: knowledgeId, embedding: { [Op.ne]: null } },
            });
