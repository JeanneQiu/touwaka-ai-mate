...
class ToolManager {
  ...
  async loadBuiltinSkill() {
    try {
      // 动态导入内置技能
      const builtinPath = path.join(__dirname, '..', 'skills', 'builtin', 'index.js');
      ...
      for (const tool of tools) {
        ...
        if (toolName) {
          this.toolToSkill.set(toolName, 'builtin');
          logger.debug(`[ToolManager] 注册内置工具: ${toolName}`);
        }
      }
  ...
  async executeTool(toolName, params, context = {}) {
    ...
    // 内置工具：直接执行
    if (skillId === 'builtin' && this.builtinSkill) {
   ...
   * 直接在主进程中执行，不走子进程
   */
  async executeBuiltinTool(toolName, params, context) {
    const startTime = Date.now();

    try {
      const result = await this.builtinSkill.execute(toolName, params, context);
      const duration = Date.now() - startTime;

      if (result.success) {
        logger.info(`[ToolManager] 内置工具执行成功: ${toolName} (${duration}ms)`);
      } else {
        logger.warn(`[ToolManager] 内置工具执行失败: ${toolName}`, result.error);
      }
      ...
      const duration = Date.now() - startTime;
      logger.error(`[ToolManager] 内置工具执行异常: ${toolName}`, error.message);
