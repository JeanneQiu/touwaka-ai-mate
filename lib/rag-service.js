...
class RAGService {
  ...
  async vectorSearch(queryEmbedding, kbIds, topK, threshold, userId) {
    ...
    for (const kbId of kbIds) {
      // 获取该知识库的所有带向量的知识点
      const points = await this.getPointsWithEmbedding(kbId, userId);

      for (const point of points) {
        try {
          const embedding = JSON.parse(point.embedding.toString());
   ...
   * 获取带向量的知识点
   */
  async getPointsWithEmbedding(kbId, userId) {
    // 验证权限
    const kb = await this.KnowledgeBase.findOne({
      where: { id: kbId, owner_id: userId },
      raw: true,
    ...
    const points = await this.KnowledgePoint.findAll({
      where: {
        knowledge_id: knowledges.map(k => k.id),
      },
      raw: true,
    });
    ...
    return points
      ...
      .map(p => ({
        ...p,
        knowledge_title: knowledgeMap.get(p.knowledge_id) || 'Unknown',
        kb_id: kbId,
        kb_name: kb.name,
      }));
  }
  ...
  formatResult(result, style) {
    if (style === 'concise') {
      return `【${result.title || '知识点'}】${result.content.substring(0, 300)}${result.content.length > 300 ? '...' : ''}`;
    }

    if (style === 'detailed') {
      return `
### ${result.title || '知识点'}
> 来源：${result.kb_name} > ${result.knowledge_title}
...
`;
    ...
    // default style
    return `【${result.kb_name} > ${result.knowledge_title}】
