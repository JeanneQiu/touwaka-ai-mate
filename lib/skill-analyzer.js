...
const SCAN_CONFIG = {
  // 代码文件扩展名
  codeExtensions: ['.js', '.py', '.ts', '.sh', '.mjs', '.cjs'],
  // 忽略的扩展名
  ignoreExtensions: ['.xsd', '.json', '.md', '.txt', '.lock', '.map'],
  // 忽略的目录
  ignoreDirs: ['node_modules', '__pycache__', '.git', 'schemas', 'validators'],
  // 单文件大小限制（50KB）
  maxFileSize: 50 * 1024,
  // 总内容大小限制（100KB）
...
请只返回 JSON，不要包含其他内容。`;
   ...
   * @returns {Promise<Array<{path: string, content: string}>>} 文件列表
   */
  async scanSkillFiles(skillDir) {
    const files = [];
    let totalSize = 0;
    ...
    try {
      ...
      try {
        ...
        if (content.length <= SCAN_CONFIG.maxFileSize) {
          files.push({ path: 'index.js', content, size: content.length });
      ...
      try {
        ...
        const scriptFiles = await this.scanDirectory(scriptsDir, skillDir, totalSize);
        files.push(...scriptFiles.files);
        totalSize = scriptFiles.totalSize;
      } catch {
        // scripts/ 不存在，忽略
      }

      logger.info('[SkillAnalyzer] 扫描完成', {
      ...
      logger.error('[SkillAnalyzer] 扫描文件失败:', error.message);
      return files;
    }
  }
   ...
   * @param {string} baseDir - 基础路径（用于计算相对路径）
   * @param {number} currentTotalSize - 当前总大小
   * @returns {Promise<{files: Array, totalSize: number}>}
   */
  async scanDirectory(dir, baseDir, currentTotalSize = 0) {
    const files = [];
    ...
    try {
      ...
      for (const entry of entries) {
        // 检查是否超过总大小限制
        if (totalSize >= SCAN_CONFIG.maxTotalSize) {
          logger.debug('[SkillAnalyzer] 达到总大小限制，停止扫描');
          break;
        }
        ...
        if (entry.isDirectory()) {
          // 跳过忽略的目录
          if (SCAN_CONFIG.ignoreDirs.includes(entry.name)) {
            continue;
          }
          // 递归扫描子目录
          const result = await this.scanDirectory(fullPath, baseDir, totalSize);
          ...
          // 跳过忽略的扩展名
          if (SCAN_CONFIG.ignoreExtensions.includes(ext)) {
            continue;
          }
          
          // 只包含代码文件
          if (!SCAN_CONFIG.codeExtensions.includes(ext)) {
            continue;
          }
          ...
          try {
            ...
            // 检查文件大小
            if (stat.size > SCAN_CONFIG.maxFileSize) {
              logger.debug(`[SkillAnalyzer] 跳过大文件: ${entry.name} (${(stat.size / 1024).toFixed(1)}KB)`);
              continue;
            }

            // 检查总大小
            if (totalSize + stat.size > SCAN_CONFIG.maxTotalSize) {
              logger.debug('[SkillAnalyzer] 达到总大小限制，跳过文件:', entry.name);
              continue;
            }
            ...
            logger.warn(`[SkillAnalyzer] 读取文件失败: ${fullPath}`, err.message);
          }
        }
      }
    } catch (error) {
      logger.warn(`[SkillAnalyzer] 扫描目录失败: ${dir}`, error.message);
    }
   ...
   * @param {Array<{path: string, content: string}>} codeFiles - 代码文件列表
   * @returns {string} 提示词
   */
  buildMultiFilePrompt(skillMd, codeFiles) {
    let codeFilesSection = '';
    ...
    return `你是一个技能分析专家。请分析以下技能文件，提取工具定义和参数。
...
1. **工具提取**：从代码文件中识别导出的函数/工具，提取名称、描述和参数
2. **安全检查**：
   - 检查是否有 eval()、Function()、vm.compile() 等动态代码执行
   - 检查是否有访问敏感文件（/etc/passwd、.env 等）的代码
   - 检查是否有向未知服务器发送数据的代码
   - 检查是否有使用 child_process 执行系统命令
3. **安全评分**：
   - 100: 完全安全
   - 80-99: 轻微警告
   - 60-79: 中等风险
