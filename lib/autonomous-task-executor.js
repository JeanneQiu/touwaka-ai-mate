...
export function createAutonomousTaskExecutor(options = {}) {
  ...
  function shouldExecute(task, lastMessageTime) {
    ...
    // 检查距离最后一条消息是否超过最小间隔（默认15分钟）
    const diffMinutes = (now - new Date(lastMessageTime)) / (1000 * 60);
  ...
  return async function autonomousTaskHandler(db) {
    ...
    try {
      ...
      for (const task of autonomousTasks) {
        ...
        // 检查是否需要执行
        if (shouldExecute(task, lastMessageTime)) {
